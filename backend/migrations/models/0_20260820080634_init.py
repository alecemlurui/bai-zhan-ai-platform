...
COMMENT ON COLUMN "tasks"."status" IS 'PENDING: pending\nRUNNING: running\nSUCCESS: success\nFAILED: failed';
...
COMMENT ON COLUMN "materials"."type" IS 'TEXT: text\nIMAGE: image';
...
COMMENT ON COLUMN "media"."type" IS 'IMAGE: image\nVIDEO: video';
...
COMMENT ON COLUMN "topics"."status" IS 'CREATED: created\nGENERATING_TITLES: generating_titles\nCOMPLETED: completed';
...
COMMENT ON COLUMN "articles"."status" IS 'DRAFT: draft\nGENERATING: generating\nCOMPLETED: completed\nPUBLISHED: published';
CREATE TABLE IF NOT EXISTS "publish_records" (
    ...
    "created_at" TIMESTAMPTZ NOT NULL,
    "updated_at" TIMESTAMPTZ NOT NULL,
    "account_id" INT REFERENCES "platform_accounts" ("id") ON DELETE CASCADE,
    "article_id" INT NOT NULL REFERENCES "articles" ("id") ON DELETE CASCADE
);
COMMENT ON COLUMN "publish_records"."status" IS 'PENDING: pending\nPROCESSING: processing\nSUCCESS: success\nFAILED: failed';
